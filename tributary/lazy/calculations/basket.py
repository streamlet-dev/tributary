...
def Len(self):
    ...
    return unary(
        self, "len({})".format(self._name_no_id), (lambda x: len(x))
    )
...
def CountBasket(self):
    """Compute len(n) for node n"""
    return unary(
        self, "count({})".format(self._name_no_id), (lambda x: len(x))
    )
...
def MaxBasket(self):
    ...
    return unary(
        self, "max({})".format(self._name_no_id), (lambda x: max(x))
    )
...
def MinBasket(self):
    """Compute max(n) for node n"""
    return unary(
        self, "min({})".format(self._name_no_id), (lambda x: min(x))
    )
...
def SumBasket(self):
    ...
    return unary(
        self, "sum({})".format(self._name_no_id), (lambda x: sum(x))
    )
